...
{
    public partial class frmConsultaVehiculosPorMarca : Form
    {
        private readonly VehiculoRepository _vehiculoRepository;
        private readonly MarcaRepository _marcaRepository;

        public frmConsultaVehiculosPorMarca()
        {
            InitializeComponent();
        ...
        private void frmConsultaVehiculosPorMarca_Load(object sender, EventArgs e)
        {
            CargarMarcas();
        }
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Por favor seleccione una marca", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            try
            {
                var vehiculos = _vehiculoRepository.GetByMarca(idMarca);
                dgvVehiculos.DataSource = vehiculos;
                ConfigurarDataGridView();

                if (vehiculos.Count == 0)
                {
                    MessageBox.Show("No hay vehículos para la marca seleccionada", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {

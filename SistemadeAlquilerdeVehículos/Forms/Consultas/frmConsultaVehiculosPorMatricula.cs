...
{
    public partial class frmConsultaVehiculosPorMatricula : Form
    {
        private readonly VehiculoRepository _vehiculoRepository;

        public frmConsultaVehiculosPorMatricula()
        {
            InitializeComponent();
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Por favor ingrese una placa para buscar", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            try
            {
                var vehiculos = _vehiculoRepository.GetByPlaca(placa);
                dgvVehiculos.DataSource = vehiculos;
                ConfigurarDataGridView();

                if (vehiculos.Count == 0)
                {
                    MessageBox.Show("No se encontraron vehículos con esa placa", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
